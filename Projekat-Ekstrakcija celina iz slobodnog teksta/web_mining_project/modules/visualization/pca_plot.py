...
def plot_pca_clusters(embeddings, labels, optimal_k, clusters):
    ...
    centers_2d = pca.transform(np.array([emb.mean(axis=0) for emb in embeddings]))
    plt.scatter(centers_2d[:, 0], centers_2d[:, 1], c='red', marker='X', s=300, edgecolors='black', linewidth=2, label='Centri')
